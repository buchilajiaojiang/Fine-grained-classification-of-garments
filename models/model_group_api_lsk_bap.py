...
class Net(nn.Module):
    def __init__(self, num_classes, num_groups = 2, hidden_c = 256):
        ...
        self.fc = nn.Linear(2048 * 32, self.num_classes) # 需要修改的值
        self.drop = nn.Dropout(p=0.5)
    ...
    def forward(self, x,flag='train'):
        ...
        if flag == 'train' or flag == 'val':
            ...
            attention = self.proj2(feature)
            attention = attention[:,:32,:,:]
            raw_features, p0 = self.bap(feature,attention) # bap怎么乘我还没确定好
            # p0 = self.flatten(self.avg(p0))
            
            p0 = self.fc(self.flatten(p0))
            
                
            return  raw_features, p0 # b 13
