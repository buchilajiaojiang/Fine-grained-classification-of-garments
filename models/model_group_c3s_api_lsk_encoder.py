...
class GroupInteractionFusion(nn.Module):
    def __init__(self, in_channels, num_groups=4):
        ...
        self.map1 = Conv(in_channels * num_groups,in_channels, 1,1)
        self.drop = nn.Dropout(p=0.5)
        # self.map2 = nn.Linear(512, in_channels)
        self.map2 = Conv(in_channels,in_channels, 1,1)
    ...
    def forward(self, x, flag='train'):
        ...
        if flag == 'train' or flag == 'val':
            ...
            #     features.append(self.adpavgpool(grouped_features[i]).squeeze())
            
            ### new
            # mutual_features = features.view(groups_num * b, inchannel, 14, 14)
            # mutual_features = torch.cat(features, dim=1) # groups_num b
            # features = self.adpavgpool(features)
            mutual_features = features.reshape(features.size(0), -1, features.size(3), features.size(4))
            map2_out = self.map1(mutual_features) # 降维
            map2_out = self.map2(self.drop(map2_out)) # 升维

            gates = [] # 4 * b * inchannel fw fh
            # gates = torch.mul(map2_out, features)
            # gates = self.sigmoid(gates)
            for i in range(self.num_groups):
                gate1 = torch.mul(map2_out, grouped_features[i])
                # gate1 = self.sigmoid(gate1)
                gates.append(self.sigmoid(gate1))
                
            featuers_all = [] # 多组特征 分组了 group group batch inchannel fw fh
            for i in range(self.num_groups):
                feature =grouped_features[i]
                groups = [] # 每一组包含的是一个特征向量和门的交互 self, other, other, other
                # others = []
                for j in range(self.num_groups):
                    a = torch.mul(gates[j], feature) + feature
                    groups.append(a)
                featuers_all.append(groups)
            
            # logit_all = [] # nparts * nparts
            # for i in range(self.num_groups):
            #     features = featuers_all[i]
            #     for j in range(self.num_groups):
            #         logit_all.append(features[j])
...
class SABlock(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = torch.cat([attn1, attn2, attn3, attn4], dim=1) # 联结
        ## SA start
        ...
        attn = attn1 * sig[:,0,:,:].unsqueeze(1) + attn2 * sig[:,1,:,:].unsqueeze(1) + attn3 * sig[:,2,:,:].unsqueeze(1) + attn4 * sig[:,3,:,:].unsqueeze(1)# 多张特征图融合
        # attn shape: b inchannel fw fh
        ## SA end
        attn = self.conv(attn) # 生成 S
        return attn # 生成 Y # default x * attn
...
class SA(nn.Module):
    def __init__(self, in_channels, num_groups=4):
        ...
        for _ in range(num_groups):  # 重复4次
            self.sa_blocks.append(SABlock(in_channels))

    def forward(self, x):
        # b, c, h, w = x.shape
        results = [] # group b inchannel fw fh
        for idx, sa in enumerate(self.sa_blocks):
            group = x[idx]
...
class Net(nn.Module):
    def __init__(self, num_classes, num_groups = 2, hidden_c = 256):
        ...
        for _ in range(num_groups):  # 重复4次
            self.convs.append(Conv(2048, hidden_c, k=1, act=False))
            
        self.convs2 = nn.ModuleList()
        for _ in range(num_groups):  # 重复4次
            self.convs2.append(Conv(2048, hidden_c, k=1, act=False))
            
        self.proj =  nn.Sequential(
                    nn.Conv2d(hidden_c * num_groups, hidden_c, kernel_size=1, bias=False),
        ...
        self.fc = nn.Linear(hidden_c, self.num_classes) # 需要修改的值
        self.drop = nn.Dropout(p=0.5)
    ...
    def forward(self, x,flag='train'):
        ...
        if flag == 'train' or flag == 'val':
            ...
            sas = self.sa(group) # group b  inchannel fw fh
            # 1. 需要让grouped_features * sas生成新特征
            sa_group = [] # group b inchannel w h
            for idx, c in enumerate(self.convs):
                gr = c(grouped_features[idx] * sas[idx])
                sa_group.append(gr + self.convs2[idx](grouped_features[idx])) # 可以考虑去掉x
                
            # 1. stack堆叠 g b c w h 最好重复2次
            # 2. -> b gc w h
            sa_f = torch.stack(sa_group, dim=0) # b 1024
            sa_f = sa_f.reshape(b, self.num_groups * self.hidden_c, h, w)
            sa_f = self.proj(sa_f) + self.short(x) 
            a1 = self.aifi(sa_f) 
            # a2 = self.aifi2(a1)
            
            # a2 = self.flatten(self.avg(a2))
            
            # result = self.fc(self.drop(a2)) # 最后预测
            
            # 1. 使用标准分类头输出
            # 2. 多预测
            a1 = self.flatten(self.avg(a1))
            
            result2 = self.fc(self.drop(a1))
            
                
            return  a1, result2 # b 13
